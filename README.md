...
- 6 starting themes (Medieval Fantasy, Sci‑Fi, Gothic Horror, Cyberpunk, Post‑Apocalyptic, Steampunk)
...
├── backend/                # Express server + Azure OpenAI integration
├── frontend/               # React + Vite UI
└── README.md
```

Key files:
- `backend/server.js` — Express app and route registration
- `backend/routes/game.js` — Game API (start, action, state, themes)
- `backend/services/openai.js` — Azure OpenAI client & prompt builder
- `frontend/src/App.jsx` — Main React app
- `frontend/src/components/` — UI components (ThemeSelector, StoryDisplay, Choices, Stats)
...
- `AZURE_OPENAI_DEPLOYMENT_NAME` — The deployment name (e.g., `GPT-41`)
...
- The backend builds a Dungeon Master prompt that includes: theme, concise system instructions for readable storytelling, character stats, inventory, and story history.
- The backend calls Azure OpenAI (via the `openai` SDK configured for Azure) and returns a JSON payload with `narration`, `choices`, `statChanges`, `itemsFound`, and `itemsUsed`.
- Backend updates the in-memory session: apply stat changes, add found items to inventory, remove used items, and append the new narration to `storyHistory`.

Note: Sessions are stored in-memory (Map). For production, use a persistent store (Redis, database).

## **API Endpoints**

- `GET /api/game/themes` — Returns available themes
- `POST /api/game/start` — Start a new game
  - Body: `{ "theme": "medieval-fantasy", "characterName": "Ada" }`
- `POST /api/game/action` — Send a player action
  - Body: `{ "sessionId": "game_xxx", "action": "Open the chest" }`
- `GET /api/game/state/:sessionId` — Get the current game state

Responses from the backend include updated `characterStats`, `inventory`, `statChanges`, `itemsFound`, and `itemsUsed`.

## **Prompt & Response Format**

The backend instructs the model to reply using a strict JSON format. Example response schema:
...
Contributions welcome. Please open issues or PRs. Keep secrets out of commits — add any keys to your local `.env` only.
